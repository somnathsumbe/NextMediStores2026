...
export async function POST(request: Request) {
  ...
  if (!user.active) return NextResponse.json({ message: "Your account is inactive." }, { status: 403 });
  const { password: _password, ...safeUser } = user;
  const response = NextResponse.json(safeUser);
  response.cookies.set("medistores_auth", String(user.id), { httpOnly: true, sameSite: "lax", secure: request.url.startsWith("https://"), path: "/", ...(body.rememberMe ? { maxAge: 60 * 60 * 24 * 30 } : {}) });
  return response;
}