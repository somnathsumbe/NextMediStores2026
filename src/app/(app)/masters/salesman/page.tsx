...
export default function SalesmanMaster() {
  ...
  const refresh = async () => {
    const next = await salesmanService.list();
    setRecords(next);
  };
  ...
  const submit = async () => {
    ...
    try {
      if (editingId === null) {
        await salesmanService.create({ fullName, mobileNumber });
      } else {
        await salesmanService.update(editingId, { fullName, mobileNumber });
      }
      await refresh();
      reset();
      setMessage(editingId === null ? "Salesman added." : "Salesman updated.");
    } catch (exception) {
      setError(exception instanceof Error ? exception.message : "Unable to save salesman.");
    }
  };
  ...
  return <div className="page">
    ...
    <div className="row g-4">
      <div className="col-xl-4"><section className="card p-3 p-lg-4 h-100">
        ...
        <div className="d-flex gap-2"><button type="button" className="btn btn-primary" onClick={() => void submit()} disabled={editingId === null && records.length >= 5}><i className="bi bi-check2 me-2" />{editingId === null ? "Add Salesman" : "Save Changes"}</button>{editingId !== null && <button type="button" className="btn btn-outline-secondary" onClick={reset}>Cancel</button>}</div>
