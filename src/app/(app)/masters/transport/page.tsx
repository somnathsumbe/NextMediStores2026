...
function uniqueValues(values: string[]) {
  return Array.from(new Map(values.map((value) => [value.trim().toLowerCase(), value.trim()])).values()).sort((left, right) => left.localeCompare(right));
}
...
export default function TransportPage() {
  ...
  const loadRecords = async () => {
    try {
      setLoading(true);
      setError("");
      const payload = await fetchJson<{ records?: TransportRecord[] }>("/api/transport");
      const nextRecords: TransportRecord[] = (payload.records ?? []).map((record) => ({
        ...record,
      ...
      setCities(uniqueValues(nextRecords.map((record) => record.city)));
      setTransportTypes(uniqueValues(nextRecords.map((record) => record.type)));
    } catch (exception) {
      setRecords([]);
      setError(exception instanceof Error ? exception.message : "Unable to load transport details.");
    } finally {
      setLoading(false);
    }
  };
  ...
  function updateForm(field: keyof TransportForm, value: string) {
    setForm((current) => ({ ...current, [field]: value }));
  }

  function openOptionModal(kind: OptionKind) {
    setOptionModal(kind);
    setOptionName("");
  }
  ...
  function saveOption(event: FormEvent<HTMLFormElement>) {
    ...
    if (!value) {
      showToast("error", `${optionModal === "city" ? "City" : "Transport Type"} is required.`);
      return;
    }
  ...
  async function submitForm(event: FormEvent<HTMLFormElement>) {
    ...
    if ([form.city, form.name, form.address, form.type].some((value) => !value.trim())) {
      showToast("error", "Please complete all required fields.");
      return;
    }

    const duplicate = records.some((record) => {
      const sameRecord = record._id === editingId || record.id === editingId;
      return !sameRecord && record.city.toLowerCase() === form.city.toLowerCase() && record.name.trim().toLowerCase() === form.name.trim().toLowerCase();
    });

    if (duplicate) {
      showToast("error", "This City and Transport Name combination already exists.");
      return;
    }

    try {
      const payload = { city: form.city.trim(), name: form.name.trim(), address: form.address.trim(), type: form.type.trim() };
      if (editingId) {
        await fetchJson(`/api/transport/${editingId}`, {
          method: "PUT",
          body: JSON.stringify(payload),
        });
        showToast("success", "Transport details updated successfully.");
      } else {
        await fetchJson("/api/transport", {
          method: "POST",
          body: JSON.stringify(payload),
        });
        showToast("success", "Transport details added successfully.");
      }

      await loadRecords();
      resetForm();
    } catch (exception) {
      showToast("error", exception instanceof Error ? exception.message : "Unable to save transport details.");
    }
  }

  function editRecord(record: TransportRecord) {
    setForm({ city: record.city, name: record.name, address: record.address, type: record.type });
    setEditingId(record._id ?? record.id);
    window.scrollTo({ top: 0, behavior: "smooth" });
  }

  async function deleteRecord(record: TransportRecord) {
    if (!window.confirm(`Delete ${record.name} transport details?`)) return;

    try {
      await fetchJson(`/api/transport/${record._id ?? record.id}`, { method: "DELETE" });
      if (editingId === (record._id ?? record.id)) resetForm();
      await loadRecords();
      showToast("success", "Transport details deleted.");
    } catch (exception) {
      showToast("error", exception instanceof Error ? exception.message : "Unable to delete transport details.");
    }
  }

  async function toggleStatus(record: TransportRecord) {
    const nextStatus = record.status === "Active" ? "Inactive" : "Active";

    try {
      await fetchJson(`/api/transport/${record._id ?? record.id}`, {
        method: "PUT",
        body: JSON.stringify({ status: nextStatus }),
      });
      await loadRecords();
      showToast("success", `${record.name} marked ${nextStatus}.`);
    } catch (exception) {
      showToast("error", exception instanceof Error ? exception.message : "Unable to update transport status.");
    }
  }
