...
export default function HsnPage() {
  ...
  const [recordToDelete, setRecordToDelete] = useState<HsnRecord | null>(null);
  ...
  function openModal(record?: HsnRecord) {
  ...
  function saveHsn(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (editingId === null) {
        hsnService.addCategory(payload.category, payload.hsnCode);
        showToast("success", "HSN Category added successfully.");
      } else {
        hsnService.update(editingId, payload);
        showToast("success", "HSN record updated.");
      }
      refresh(); closeModal();
    } catch (error) { showToast("error", error instanceof Error ? error.message : "Unable to save HSN record."); }
  }
  function editRecord(record: HsnRecord) { openModal(record); }
  ...
  function confirmDelete() {
    ...
    try {
      hsnService.delete(recordToDelete.id);
