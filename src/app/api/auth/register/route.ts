...
export async function POST(request: Request) {
  ...
  const email = data.email.trim().toLowerCase(); const mobile = data.mobile.trim();
  if (users.some(user => user.email.toLowerCase() === email || user.mobile === mobile)) return NextResponse.json({ message: "An account with this email or mobile number already exists." }, { status: 409 });
  const { confirmPassword: _confirmPassword, ...user } = data;
  users.push({ ...user, email, mobile, createdAt: new Date().toISOString() });
